...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: symptomPatterns } = await supabase
      .from('disease_symptom_patterns')
      .select('*')
      .in('symptom_name', normalizedSymptoms);

    let diseasePatterns: DiseasePattern[] = [];

    if (symptomPatterns && symptomPatterns.length > 0) {
      console.log('Using trained symptom patterns from database');

      const diseaseScoringMap = new Map<string, { score: number; matchCount: number }>();

      symptomPatterns.forEach(pattern => {
        const current = diseaseScoringMap.get(pattern.disease_name) || { score: 0, matchCount: 0 };
        diseaseScoringMap.set(pattern.disease_name, {
          score: current.score + (pattern.frequency * pattern.importance),
          matchCount: current.matchCount + 1
        });
      });

      diseasePatterns = Array.from(diseaseScoringMap.entries()).map(([disease, data]) => ({
        disease,
        keywords: normalizedSymptoms,
        threshold: 0.3,
        scoring_rules: { [disease]: data.score }
      }));

    } else {
      console.log('Using fallback rule-based patterns');
    ...
    const diseaseScores = diseasePatterns.map(pattern => {
      ...
      const score = pattern.scoring_rules
        ? Object.values(pattern.scoring_rules)[0]
        : matches / pattern.keywords.length;

      return {
        disease: pattern.disease,
        score,
        matches,
        meetsThreshold: pattern.scoring_rules ? score >= pattern.threshold : matches >= pattern.threshold
      };
    });

    const validPredictions = diseaseScores.filter(d => d.meetsThreshold);
    const bestMatch = validPredictions.length > 0
      ? validPredictions.reduce((best, current) => current.score > best.score ? current : best)
      : diseaseScores.reduce((best, current) => current.matches > best.matches ? current : best);

    const predictedDisease = bestMatch.disease;
    const confidenceScore = Math.min(bestMatch.score * 1.5, 1);
